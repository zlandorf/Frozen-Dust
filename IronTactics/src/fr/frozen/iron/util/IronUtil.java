...
public class IronUtil {
	...
	public static String findName() {
		...
		if (name == null)
			return IronConst.defaultName;
		return name;
	}

	public static void saveMap(IronMap map, File file) throws IOException {
	...
	public static IronMap loadMap(File file) throws IOException {
		...
		byte[] mapData = new byte[(int) length];

		int offset = 0;
		int numRead = 0;
		while (offset < mapData.length
				&& (numRead = inputStream.read(mapData, offset, mapData.length
						- offset)) >= 0) {
			offset += numRead;
		}
		...
		try {
			map.unserialize(mapData);
		} catch (Exception e) {
			System.out.println("map didnt unserialize correctly");
			e.printStackTrace();
			throw new IOException("file not serialized properly");
		}
		return map;
	}

	public static int getDamage(IronUnit src, IronUnit dst, boolean meleeAttack) {
		Weapon weapon = meleeAttack ? src.getMeleeWeapon() : src
				.getRangedWeapon();
		if (weapon == null)
			return 0;
		...
		if (weapon.isMagical()) {
			damage += damage
					* (float) (src.getStats().getIntelligence() / 10.0);
		} else { // physical
			if (meleeAttack) {
				damage += damage
						* (float) (src.getStats().getStrength() / 10.0);
			} else {
		...
		if (dst.getShield() != null) {
			armorValue += weapon.isMagical() ? dst.getShield()
					.getMagicalArmor() : dst.getShield().getPhysicalArmor();
		}
	...
	public static String findOptionValue(String optionName) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				if (line.length() > optionName.length()
						&& line.startsWith(optionName)) {
			...
			Logger.getLogger(IronUtil.class).warn(
					"error when trying to find " + option + "  : "
	...
	public static void saveOptionValue(String option, String value) {
		...
		try {
			...
			Logger.getLogger(IronUtil.class).error(
					"error in saveOption when saving " + option + value + " :"
							+ e.getLocalizedMessage());
		}
		Logger.getLogger(IronUtil.class).info(
				"saved " + option + value + " in file " + filePath);
	}
	...
	public static List<Vector2f> getHorizontalIntersections(IronWorld world,
			int x1, int y1, int x2, int y2, double angle) {
		...
		int nbIntersections = (int) (y2 / IronConst.TILE_HEIGHT)
				- (int) (y1 / IronConst.TILE_HEIGHT);
		...
		for (int i = 0; i < nbIntersections; i++) {
			...
			Vector2f ref2 = new Vector2f(IronConst.MAP_WIDTH
					* IronConst.TILE_WIDTH - 1, Py);
	...
	public static boolean checkGrid(IronWorld world, int x1, int y1, int x2,
			int y2, boolean shootOver) {
		Vector2f A, B;
		...
		List<Vector2f> points = getHorizontalIntersections(world, x1, y1, x2,
				y2, angle);
		...
		main: for (int i = 1; i < points.size(); i++) {
			...
			nbIntersections = (int) (B.getX() / IronConst.TILE_WIDTH)
					- (int) (A.getX() / IronConst.TILE_WIDTH);
			nbIntersections = Math.abs(nbIntersections);
			...
			for (int j = 0; j < nbIntersections; j++) {
				...
				while (P == null && Px >= 0
						&& Px < IronConst.MAP_WIDTH * IronConst.TILE_WIDTH) {
					ref1 = new Vector2f(Px, 0);
					ref2 = new Vector2f(Px, IronConst.MAP_HEIGHT
				...
				squareX = (int) Math.min(P.getX() / IronConst.TILE_WIDTH, tmpX
						/ IronConst.TILE_WIDTH);
				squareY = (int) Math.min(P.getY() / IronConst.TILE_HEIGHT, tmpY
				...
				if (squareX < 0 || squareX >= IronConst.MAP_WIDTH
						|| squareY < 0 || squareY >= IronConst.MAP_HEIGHT)
					break main;

				if (squareX == x1 / IronConst.TILE_WIDTH
						&& squareY == y1 / IronConst.TILE_HEIGHT)
					continue;
				if (squareX == x2 / IronConst.TILE_WIDTH
						&& squareY == y2 / IronConst.TILE_HEIGHT)
					break main;

				if (shootOver) {
					if (!world.getMap().getTile(squareX, squareY)
							.canShootOver()) {
						return false;
					}
				} else {
			...
			squareX = (int) Math.min(B.getX() / IronConst.TILE_WIDTH, tmpX
					/ IronConst.TILE_WIDTH);
			squareY = (int) Math.min(B.getY() / IronConst.TILE_HEIGHT, tmpY
					/ IronConst.TILE_HEIGHT);

			if (squareX < 0 || squareX >= IronConst.MAP_WIDTH || squareY < 0
					|| squareY >= IronConst.MAP_HEIGHT)
				break main;
	...
	public static double getAngle(Vector2f vec1, Vector2f vec2) {
		 ...
		 * oldversion :
		 * 
		 * double distanceV1 = IronUtil.distance(0, 0, (int)vec1.getX(),
		 * (int)vec1.getY()); double distanceV2 = IronUtil.distance(0, 0,
		 * (int)vec2.getX(), (int)vec2.getY());
		 * 
		 * double angle = Math.acos((vec1.getX() * vec2.getX() + vec1.getY() *
		 * vec2.getY()) / (distanceV1 * distanceV2));//angle between vec and
		 * (0,-1)
		 */
		double distanceV1 = vec1.length();
		double distanceV2 = vec2.length();
		if (distanceV1 == 0 || distanceV2 == 0)
			return 0;
		double angle = Math.acos(Vector2f.dot(vec1, vec2)
				/ (distanceV1 * distanceV2));
		return angle;
	}
	...
	public static Vector2f getIntersectionPoint(Vector2f A, Vector2f B,
		...
		double m = -(-I[0] * A.getY() + I[0] * C.getY() + I[1] * A.getX() - I[1]
				* C.getX())
				/ denom;
		double k = -(A.getX() * J[1] - C.getX() * J[1] - J[0] * A.getY() + J[0]
				* C.getY())
				/ denom;
	...
	public static int byteArrayToInt(byte[] array) {
		if (array.length != 4) {
			Logger.getLogger(IronUtil.class).error(
					"wrong array length cannot parse it to int");
			return 0;
		}
		int val = 0;
		for (int i = 0; i < 4; i++) {
	...
	public static byte[] readToArray(DataInputStream is, int length)
			throws IOException {
		...
		if (totalRead != length)
			throw new IOException("problem not as many bytes as expected");
