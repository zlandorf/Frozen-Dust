...
public class IronUnit extends GameObject implements Mover {
	/*
	 * serializes as : id - type - ownerId - float x - float y
	 */
	...
	public static String[] typeNames = { "footsoldier", "archer", "cavalry",
			"healer", "mage", "special" };
	...
	public IronUnit(IronWorld world, int id, int type, int ownerId, float x,
			float y) {
		super(null, x, y);
		this.id = id;
		this.type = type;
		...
		if (world != null && world.getContext() != null)
			findStatsFromXml();
	}

	public static IronUnit getUnit(char letter, IronWorld world, int entId,
			int clientId, int x, int y) {
		switch (letter) {
		case 'F':
	...
	public static IronUnit getUnit(int type, IronWorld world, int entId,
			int clientId, int x, int y) {
		switch (type) {
	...
	public void onStartTurn(boolean addParticle) {
		...
		if (getStats().getMaxMana() >= 0 && !isDead()) {
			getStats().setMana(
					(int) (manaBefore + getStats().intelligence
							* IronConst.INTELLIGENCE_REGEN_FACTOR));

			if (!playedLastTurn)
				getStats().setMana(
						(int) (getStats().getMana() + getStats().intelligence
								* IronConst.INTELLIGENCE_IDLE_REGEN_FACTOR));

			int manaAfter = getStats().getMana();
			if (manaAfter - manaBefore != 0 && addParticle) {
				GameObject manaParticle = new ManaParticle(getWorld(), getX()
						* IronConst.TILE_WIDTH + IronConst.TILE_WIDTH / 2,
	...
	public void move(int x, int y, int cost) {
		...
		world.getMap().getTile((int) _pos.getX(), (int) _pos.getY())
				.setUnitOnTile(null);
		_pos.set(x, y);
		world.getMap().getTile((int) _pos.getX(), (int) _pos.getY())
				.setUnitOnTile(this);
	...
	public void findSprite() {
		...
		String teamColorSpriteName = typeNames[type] + "_" + getRaceStr()
				+ "_color";
		if (_spriteManager.isSpriteLoaded(teamColorSpriteName)) {
			teamColorSprite = _spriteManager.getSprite(teamColorSpriteName);
		} else {
			teamColorSprite = _spriteManager.getSprite(typeNames[type]
					+ "_color");
		}

		if (teamColorSprite != null) {
			teamColorSprite.setColor(world.getContext().getPlayerInfo(ownerId)
					.getColor());
		}
		...
		/*
		 * if (weapon != null) { //weaponSprite =
		 * ISpriteManager.getInstance().getSprite("weapon_"+weapon.getName()); }
		 * else { System.out.println("WEAPON NULL, NOT GETTING ANY SPRITE"); }
		 */
	}
	...
	public String getRaceStr() {
		return IronUtil.getRaceStr(world.getContext().getPlayerInfo(ownerId)
				.getRace());
	}
	...
	public void findStatsFromXml() {
		...
		if (racestr == null) {
			Logger.getLogger(getClass()).error(
					"PROBLEM WHILE GETTING UNIT STATS");
			return;
		}
		...
		int intelligence = Integer.parseInt(ic.getAttributeValue(base,
				"intelligence"));
		int maxMana = Integer.parseInt(ic.getAttributeValue(base, "maxmana"));
		int maxMovement = IronConst.MOVE_COST_DEFAULT
				* Integer.parseInt(ic.getAttributeValue(base, "movement"));
		...
		if (equipment.getMeleeWeapon() == null) {
			...
			if (weaponName != null && !weaponName.equals(""))
				setMeleeWeapon(EquipmentManager.getInstance().getWeapon(
						weaponName));
		}
		...
		if (equipment.getRangedWeapon() == null) {
			...
			if (weaponName != null && !weaponName.equals("")) {
				setRangedWeapon(EquipmentManager.getInstance().getWeapon(
						weaponName));
			}
		}
	...
	public byte[] serialize() throws IOException {
		...
		byteArray.write(IronUtil.intToByteArray((int) _pos.x));// supposed to be
																// float, but we
																// will use as
																// int
	...
	public int getBloodState() {
		...
		float perc = hp / maxHp;
		int nbStates = bloodFx.getFrames().size() + 1;// i consider 0 is the
	...
	public void render(float deltaTime) {
		if (_sprite != null) {
			...
			/*
			 * if (weapon != null && weaponSprite != null) { if
			 * (weapon.isDisplayIdle()) { weaponSprite.draw(_pos.x *
			 * IronConfig.TILE_WIDTH, _pos.y * IronConfig.TILE_HEIGHT); } }
			 */
		} else {
			findSprite();
		}
	...
	public void renderStatusBars(float deltaTime, float x, float y) {
		...
		IronGL.drawRect(x, y, IronConst.TILE_WIDTH, height, 0.1f, 0.1f, 0.1f,
				1f);
		...
		IronGL.drawRect(x + 3, y + 3, IronConst.TILE_WIDTH - 6, height - 6, 0f,
				0f, 0f, 1f);

		float percentHealthLeft = (float) stats.getHp() / stats.getMaxHp();

		float healthWidth = percentHealthLeft * (IronConst.TILE_WIDTH - 6);
		IronGL.drawRect(x + 3, y + 3, healthWidth, 2, 1 - percentHealthLeft,
				percentHealthLeft, 0f, 1f);

		if (getStats().getMaxMana() > 0) {
			float percentManaLeft = (float) stats.getMana()
					/ stats.getMaxMana();
	...
	public void renderTileGfx(float deltaTime, int playerTurnId) {
		if (!hasPlayed() && playerTurnId >= 0 && playerTurnId == getOwnerId()) {
			// TODO : change color if it is not my turn ?!

			IronGL.drawRect((int) getX() * IronConst.TILE_WIDTH, (int) getY()
					* IronConst.TILE_HEIGHT, IronConst.TILE_WIDTH,
					IronConst.TILE_HEIGHT, 1f, 0.8f, 0f, 0.25f);

			if (isSelected()) {
				IronGL.drawRect((int) _pos.x * IronConst.TILE_WIDTH,
						(int) _pos.y * IronConst.TILE_HEIGHT,
						IronConst.TILE_WIDTH, IronConst.TILE_HEIGHT, 1f, 0f,
						0f, 0.5f);
			}// end if isSelected()
		}
	}

	public void renderActionTiles(int playerTurnId) {
		if (!hasPlayed() && isSelected() && playerTurnId >= 0
				&& playerTurnId == getOwnerId()) {
			...
			for (IronUnit unit : world.getUnits()) {
				...
				for (Skill skill : getSkills()) {
					if (skill.getSkillType() == Skill.SHIELD_BLOCK)
						continue;
					canDo |= skill.canDo(getWorld(), getId(),
							(int) unit.getX(), (int) unit.getY());
				}

				if (canDo) {
					int[] pos = new int[] { (int) unit.getX(),
							(int) unit.getY() };
					actionCache.add(pos);
				}
			}
			...
			for (int[] p : actionCache) {
				IronGL.drawRect(p[0] * IronConst.TILE_WIDTH, p[1]
						* IronConst.TILE_HEIGHT, IronConst.TILE_WIDTH,
						IronConst.TILE_HEIGHT, 1f, 0.1f, 1f, 0.5f);
			}
		}
	}

	public void renderMoveableTiles(int playerTurnId) {
		if (!hasPlayed() && isSelected() && playerTurnId >= 0
				&& playerTurnId == getOwnerId()) {

			// if (_pos.getX() != movementCachePos[0] || _pos.getY() !=
			// movementCachePos[1]) {
			// cache needs to be computed
			int xstart, ystart, xend, yend;
			movementCache.clear();

			xstart = Math.max(0, (int) _pos.getX() - getMovement()
					/ IronConst.MOVE_COST_DEFAULT);
			ystart = Math.max(0, (int) _pos.getY() - getMovement()
					/ IronConst.MOVE_COST_DEFAULT);

			xend = Math.min(IronConst.MAP_WIDTH - 1, (int) _pos.getX()
					+ getMovement() / IronConst.MOVE_COST_DEFAULT);
			yend = Math.min(IronConst.MAP_HEIGHT - 1, (int) _pos.getY()
					+ getMovement() / IronConst.MOVE_COST_DEFAULT);

			for (int i = xstart; i <= xend; i++) {
				for (int j = ystart; j <= yend; j++) {
					if (i == _pos.getX() && j == _pos.getY())
						continue;
			...
			for (int[] p : movementCache) {
				IronGL.drawRect(p[0] * IronConst.TILE_WIDTH, p[1]
						* IronConst.TILE_HEIGHT, IronConst.TILE_WIDTH,
						IronConst.TILE_HEIGHT, 0.3f, 1f, 0f, 0.2f);
			}
		}
	}

	public void renderStatsInGui(float deltaTime, float x1, float y1, float w,
			float h) {
