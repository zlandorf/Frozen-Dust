...
public class IronMap implements TileBasedMap {
	...
	public IronMap() {
		...
		for (int i = 0; i < tiles.length; i++) {
			for (int j = 0; j < tiles[i].length; j++) {
				...
				tiles[i][j].setSubType(findSubType(Tile.tile_names[tiles[i][j]
						.getType()]));
			}
		}
	}
	...
	private boolean isDeployementZone(int x, int y) {
		int xstart = (IronConst.MAP_WIDTH - 10) / 2 /*
													 * + (IronConfig.MAP_WIDTH &
													 * 1)
													 */;
		int xend = IronConst.MAP_WIDTH - (IronConst.MAP_WIDTH - 10) / 2 - 1;
		boolean inX = x >= xstart && x <= xend;
		boolean inY = (y >= 0 && y < 4)
				|| (y >= IronConst.MAP_HEIGHT - 4 && y < IronConst.MAP_HEIGHT);
		return inX && inY;
	}

	private void generateElement(int type, double percentage,
			double persistence, int octave, boolean respectDeployement,
			boolean isObject) {
				...
				if (vals[i][j] < threshold) {
					...
					if (isObject) {
						if (!tiles[i][j].isOccupied()) {
							...
							tiles[i][j]
									.setObjectSubType(findSubType(Tile.object_names[type]));
						...
						if (type == Tile.TYPE_WATER) {
							if (waterAround(i, j, vals, threshold)) {
								...
								tiles[i][j]
										.setSubType(findSubType(Tile.tile_names[type]));
								tiles[i][j].setOccupied(true);
							}
						} else {
							tiles[i][j].setType(type);
	...
	private int findSubType(String name) {
		String nbSubTypes = IronConfig.getIronXMLParser().getAttributeValue(
				"map/" + name, "nbtypes");
		...
		if (nbSubTypes != null) {
			...
			// for the moment, chance is the chance of falling on a non default
			// square, the default square being _11

			String chanceStr = IronConfig.getIronXMLParser().getAttributeValue(
					"map/" + name, "chance");
			if (chanceStr != null) {
				chance = Float.parseFloat(chanceStr);
			}
			// TODO change this for every subsprite to have its own probability
			// to appear
	...
	private boolean waterAround(int x, int y, double[][] vals, double threshold) {
		...
		if (x > 0 && vals[x - 1][y] < threshold && !isDeployementZone(x - 1, y)
				|| x < IronConst.MAP_WIDTH - 1 && vals[x + 1][y] < threshold
				&& !isDeployementZone(x + 1, y) || y > 0
				&& vals[x][y - 1] < threshold && !isDeployementZone(x, y - 1)
				|| y < IronConst.MAP_HEIGHT - 1 && vals[x][y + 1] < threshold
				&& !isDeployementZone(x, y + 1)) {
			return true;
		}
		...
		for (int i = 0; i < tiles.length; i++) {
			for (int j = 0; j < tiles[i].length; j++) {
				...
				if (i > 0 && j > 0 && tiles[i-1][j-1].getType() > type) {
					tiles[i-1][j-1].setTerrainOverlay(tiles[i-1][j-1].getTerrainOverlap() | Tile.SE);
				}

				if (i > 0 && tiles[i-1][j].getType() > type) {
					tiles[i-1][j].setTerrainOverlay(tiles[i-1][j].getTerrainOverlap() | Tile.E);
				}

				if (i > 0 && j < IronConst.MAP_HEIGHT - 1 && tiles[i-1][j+1].getType() > type) {
					tiles[i-1][j+1].setTerrainOverlay(tiles[i-1][j+1].getTerrainOverlap() | Tile.NE);
				}

				if (j < IronConst.MAP_HEIGHT - 1 && tiles[i][j+1].getType() > type) {
					tiles[i][j+1].setTerrainOverlay(tiles[i][j+1].getTerrainOverlap() | Tile.N);
				}

				if (i < IronConst.MAP_WIDTH - 1 && j < IronConst.MAP_HEIGHT - 1 && tiles[i+1][j+1].getType() > type) {
					tiles[i+1][j+1].setTerrainOverlay(tiles[i+1][j+1].getTerrainOverlap() | Tile.NW);
				}

				if (i < IronConst.MAP_WIDTH - 1 && tiles[i+1][j].getType() > type) {
					tiles[i+1][j].setTerrainOverlay(tiles[i+1][j].getTerrainOverlap() | Tile.W);
				}

				if (i < IronConst.MAP_WIDTH - 1 && j > 0 && tiles[i+1][j-1].getType() > type) {
					tiles[i+1][j-1].setTerrainOverlay(tiles[i+1][j-1].getTerrainOverlap() | Tile.SW);
				}

				if (j > 0 && tiles[i][j-1].getType() > type) {
					tiles[i][j-1].setTerrainOverlay(tiles[i][j-1].getTerrainOverlap() | Tile.S);
				}
			}
		}
	}
		...
		/*for (int i = 0; i < IronConst.MAP_WIDTH; i++) {
			for (int j = 0; j < IronConst.MAP_HEIGHT; j++) {
				IronGL.drawLine(0, j * IronConst.TILE_HEIGHT,
						IronConst.MAP_WIDTH * IronConst.TILE_WIDTH, j
								* IronConst.TILE_HEIGHT);
			}
			IronGL.drawLine(i * IronConst.TILE_WIDTH, 0, i
					* IronConst.TILE_WIDTH, IronConst.MAP_HEIGHT
					* IronConst.TILE_HEIGHT);
		}*/
		IronGL.drawGrid(0, 0, IronConst.MAP_WIDTH * IronConst.TILE_WIDTH,
				IronConst.MAP_HEIGHT * IronConst.TILE_HEIGHT,
				IronConst.TILE_WIDTH, IronConst.TILE_WIDTH);
	}
	...
	public static void main(String[] args) {
		...
		/*
		 * IronMap map = new IronMap(); map.generateMap();
		 */
		/*
		 * System.out.println("gererated map : \n"+map); try { byte[] data; data
		 * = map.serialize(); map.unserialize(data);
		 * System.out.println("\n\nunserialized : \n"+map); } catch (IOException
		 * e) { e.printStackTrace(); }
		 */
	}
	...
	//used by map editor when a tile is changed
	public void resetSprites() {
		for (int i = 0; i < getTiles().length; i++) {
			for (int j = 0; j < getTiles()[i].length; j++) {
				getTiles()[i][j].resetSprites();
			}
		}
		initSprites();
	}
	...
	public void resetOverLays() {
		for (int i = 0; i < getTiles().length; i++) {
			for (int j = 0; j < getTiles()[i].length; j++) {
				getTiles()[i][j].resetTerrainOverlay();
			}
		}
	}
}
