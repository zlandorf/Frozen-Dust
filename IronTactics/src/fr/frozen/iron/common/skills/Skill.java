...
public abstract class Skill {
	...
	public abstract List<int[]> computeSkill(IronWorld world, int srcId, int x,
			int y);// returns list of couple [unitId, value] without changing
					// world state
	...
	public List<int[]> executeSkill(IronWorld world, int srcId, int x,
			int y) {// returns list of couple [unitId, value] and changes world
		// state
		List<int[]> res = computeSkill(world, srcId, x, y);
	...
	protected void executeCommon(IronWorld world, int srcId, int x, int y,
		...
		for (int[] couple : values) {
			...
			Logger.getLogger(getClass()).info(
					src + " uses " + getSkillName() + " on " + target + " for "
							+ couple[1] + " damage");
		}
	}
	...
	public void executeClientSide(IronWorld world, int srcId, int x, int y,
		...
		for (int[] couple : values) {
			...
			damage = new DamageParticle(world, target.getX()
					* IronConst.TILE_WIDTH + IronConst.TILE_WIDTH / 2, target
					.getY()
					* IronConst.TILE_WIDTH, couple[1]);
			...
			if (couple[1] < 0) {
				...
				if (couple[1] <= -100) {
					impactSound = SoundManager.getInstance().getSound(
							"strong_hit");
				} else {
					float armorValue = 0;
		...
		if (src.getStats().getMaxMana() > 0) {
			...
			if (manaCost != 0) {
				ManaParticle mp = new ManaParticle(world, src.getX()
						* IronConst.TILE_WIDTH + IronConst.TILE_WIDTH / 2, src
						.getY()
						* IronConst.TILE_WIDTH, manaCost);
